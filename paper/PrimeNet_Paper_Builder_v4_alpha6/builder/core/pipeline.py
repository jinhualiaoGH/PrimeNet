...
from builder.figures import FigureCatalog, FigureRenderer, FigureSpecLoader
from builder.sections import ManuscriptAssembler, SectionCatalog, SectionRenderer, SectionSpecLoader
...
class BuildPipeline:
    ...
    @classmethod
    def _figures(cls, context: BuildContext) -> StageResult:
        loader, registry = cls._load_evidence(context)
        specs = FigureSpecLoader(context.plugin.root).discover()
        renderer = FigureRenderer(registry, loader)
        output_root = context.build_dir / "figures"
        rendered = tuple(renderer.render(spec, output_root) for spec in specs)
        catalog = FigureCatalog.write(rendered, context.build_dir / "figure_catalog.json")
        return StageResult("figures", "passed", f"{len(rendered)} figures; catalog={catalog}")


    @classmethod
    def _sections(cls, context: BuildContext) -> StageResult:
        loader, registry = cls._load_evidence(context)
        specs = SectionSpecLoader(context.plugin.root).discover()
        renderer = SectionRenderer(
            registry, loader, context.plugin.root, context.build_dir, strict=context.configuration.strict
        )
        output_root = context.build_dir / "sections"
        rendered = tuple(renderer.render(spec, output_root) for spec in specs)
        catalog = SectionCatalog.write(rendered, context.build_dir / "section_catalog.json")
        manuscript = ManuscriptAssembler.assemble(rendered, context.build_dir / "manuscript.md")
        unresolved = sum(item.unresolved_count for item in rendered)
        status = "passed" if unresolved == 0 else "warning"
        return StageResult(
            "sections", status, f"{len(rendered)} sections; unresolved={unresolved}; catalog={catalog}; manuscript={manuscript}"
        )
    ...
    def _summarize(context: BuildContext) -> StageResult:
        marker = context.build_dir / "SECTION_ENGINE_READY.txt"
        message = "PrimeNet Paper Builder v4 Section Engine executed successfully.\n"
