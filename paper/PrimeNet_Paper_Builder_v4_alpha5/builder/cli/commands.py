...
def execute(args: Namespace) -> int:
    ...
    if args.command == "evidence":
        loader = EvidenceLoader(cfg.evidence_root)
        registry = EvidenceRegistry.from_packages(loader.discover())
        report = EvidenceValidator(loader).validate(registry)
        print(report.render())
        catalog_path = EvidenceCatalog(registry, loader).write(cfg.output_root / "evidence_catalog.json")
    ...
    if args.list_papers:
        ...
        return 0
    if not cfg.paper:
        raise ValueError("--paper is required unless --list-papers, doctor, or evidence is used")
    result = app.build()
