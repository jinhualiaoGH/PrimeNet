...
def _validate_summary(
    ...
    if missing:
        raise ValueError(
            "Information summary is missing required fields: "
            + ", ".join(sorted(missing))
            + "."
        )
    ...
    if summary["observatory"] != OBSERVATORY_NAME:
        raise ValueError(
            f"summary.observatory must be {OBSERVATORY_NAME!r}."
        )
    ...
    if expected_kind is not None and kind != expected_kind:
        raise ValueError(
            f"Expected Information summary kind {expected_kind!r}; received {kind!r}."
        )

    missing_kind = _KIND_REQUIRED_FIELDS[kind].difference(summary)
    if missing_kind:
        raise ValueError(
            f"{kind} summary is missing required fields: "
            + ", ".join(sorted(missing_kind))
            + "."
        )
    ...
    if start != coordinate_range.start or end != coordinate_range.end:
        raise ValueError(
            ...
            f"summary={start}..{end}, "
            f"coordinate_range={coordinate_range.start}..{coordinate_range.end}."
        )
...
def _measurements(summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for key in (
        ...
        "gap_count",
        "selected_state_count",
        "event_language_length",
        "event_count",
        "total_selected_transitions",
        "nonzero_source_rows",
    ):
        if key in summary:
    ...
    if gap_summary is not None:
        gap_mapping = _require_mapping(gap_summary, "gap_summary")
        if "gap_count" in gap_mapping:
            result.setdefault("gap_count", gap_mapping["gap_count"])

    taxonomy_summary = summary.get("taxonomy_summary")
    if taxonomy_summary is not None:
        taxonomy_mapping = _require_mapping(taxonomy_summary, "taxonomy_summary")
        if "gap_count" in taxonomy_mapping:
            result.setdefault("classified_gap_count", taxonomy_mapping["gap_count"])
...
def build_information_observation(
    *,
    observation_id: str,
    coordinate_range: CoordinateRange,
    summary: Mapping[str, Any],
    ...
    """Map one accepted Information Observatory summary to an Observation.

    The builder performs no scientific computation and no file-system operations.
    It validates identity and range, then preserves the existing scientific summary.
    """
