...
def build_docx(root,config,stats,tables):
 ...
 for sec in section_paragraphs(stats):
  ...
  if sec.get('table'):
   ...
   table_name=sec['table']; table_rows=tables.get(table_name); print(f'  [SKIP] manuscript table {table_name} (unavailable)') if table_rows is None else _add_table(doc,table_rows,table_font)
  doc.add_page_break()
